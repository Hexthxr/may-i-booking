...
// Clamp to stock if given (undefined => ignore)
function clampToStock(qty, stock) {
  const q = Math.max(1, Number(qty)||1);
  if (typeof stock === 'undefined' || stock === null) return q;
...
export function addToLocalCart(newItems) {
  ...
  const map = new Map(getLocalCart().map(it => [String(it.bookId), { ...it }]));

  for (const it of list) {
    const id = String(it.bookId);
    const before = map.get(id)?.qty || 0;
    const want = Math.max(1, Number(it.qty)||1);
    // try to use it.stock if provided
    const stock = (typeof it.stock !== 'undefined') ? Number(it.stock) : undefined;
    const target = clampToStock(before + want, stock);
    if (target === 0) continue;
    const meta = map.get(id) || {};
    map.set(id, {
      ...meta,
      bookId: id,
      title: it.title || meta.title || id,
      price: Number(it.price || meta.price || 0),
      qty: target,
      coverUrl: it.coverUrl || meta.coverUrl || '',
    });
  }
  const merged = Array.from(map.values());
  setLocalCart(merged);
...
export function setQtyLocal(bookId, qty, stock) {
  const target = clampToStock(qty, stock);
  if (target === 0) {
    return removeLocalItems([bookId]);
  }
  const next = getLocalCart().map(it => String(it.bookId) === String(bookId) ? { ...it, qty: target } : it);
  setLocalCart(next);
  return next;
}
